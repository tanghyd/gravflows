...
from gwpe.pytorch.datasets import WaveformDataset
...
def train_distributed(
    ...
    mean = torch.tensor(generator.statistics['mean'].values, device=rank, dtype=torch.float32)
    std = torch.tensor(generator.statistics['std'].values, device=rank, dtype=torch.float32)
    ...
    encoder = BasisEncoder(basis_dir, static_args_ini, preload=False)
    encoder.load(mmap_mode='c')
    encoder.basis.truncate(n)
    encoder.to(device)
        ...
        if rank != 3:  # gpu3 will sample from training samples
            with torch.no_grad():
                if rank == 0:
                    ...
                    context = torch.tensor(np.stack(list(event_strain.values()))[None], dtype=torch.complex64, device=rank)
                    gts = torch.zeros((1,len(val_dataset.parameters.columns)), dtype=torch.float32, device=rank)  # dummy tensor
                    ...
                    context = torch.tensor(np.array(context), dtype=torch.complex64, device=rank)
                    ...
                    gts = torch.tensor(val_dataset.parameters.iloc[[idx]].values, dtype=torch.float32, device=rank)
                    ...
                    gts = torch.tensor(test_dataset.parameters.iloc[[idx]].values, dtype=torch.float32, device=rank)
                    
                context = encoder(context)
                # context = torch.einsum('bij, bijk -> bik', context, encoder.V) * encoder.scaler
    ...
    with tqdm(total=len(dataloader)*epochs, disable=disable_pbar) as progress:
        for epoch in range(1, 1+epochs):
            ...
            coefficients = encoder.time_translate(coefficients, parameters[:, generator.parameters.index('time')])

            coefficients = coefficients.to(rank, non_blocking=True)
            # projections = projections.to(rank, non_blocking=True)
            ...
            if (interval != 0) and (epoch % interval == 0):
                ...
                with torch.no_grad():
                    ...
                    projections, parameters = next(iterator)
                    projections = projections.to(rank, dtype=torch.complex64, non_blocking=True)
                    parameters = parameters.to(rank, dtype=torch.float32, non_blocking=True)
