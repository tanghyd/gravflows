...
class LFIGWDataset(Dataset):
    def __init__(
        ...
        self.parameters = pd.read_csv(self.data_dir / 'parameters.csv', index_col=0).iloc[:90000]
    ...
    def _worker_init_fn(self, worker_id: int=None):
        ...
        self.data = np.load(self.data_dir / self.data_file, mmap_mode='r')[:90000]
        ...
        if self.psd_dir is not None:            
            ...
            if self.ref_ifo is not None:
                ...
                self.whiten = np.concatenate(self.whiten).astype(self.complex_dtype)
...
#         self.std = self.parameters.std(axis=0).values
        ...
        # analytic computation - check whether psi is treated correctly relative to samples
        ...
        # data = np.load(self.data_dir / self.data_file, mmap_mode='r')  # memmap's can't be passed to workers
        # n_samples = 100000
        # samples = self.extrinsics.draw(n_samples)
        # projected_coefficients = np.empty((n_samples, len(self.ifos), 100), dtype=data.dtype)
        # for i, ifo in enumerate(self.ifos):
    ...
    def _worker_init_fn(self, worker_id: int=None):
        ...
        if self.psd_dir is not None:
            ...
            for ifo in ('H1', 'L1'):
                ...
                self.psds[ifo] = psd[:self.static_args['fd_length']].data
            
            if self.ref_ifo is not None:
                # get relative whitening matrix for reduced basis (pre-computed freq transform)
                self.whiten = []
                for ifo in self.psds:
                    whiten = (self.psds[self.ref_ifo] / self.psds[ifo]) ** 0.5  # relative whiten ratio
                    whiten = ((self.encoder.basis.Vh * whiten) @ self.encoder.basis.V)  # whiten RB
                    self.whiten.append(whiten)

                self.whiten = np.concatenate(self.whiten).astype(self.complex_dtype)
                self.whiten = torch.from_numpy(self.whiten)  # better to do this in collate_fn?
                
    def _collate_fn(self, batch: Tuple[np.ndarray], extrinsics=None):
        # get data
        waveforms = np.stack([item[0] for item in batch]).astype(self.complex_dtype)
        intrinsics = np.stack([item[1] for item in batch])
        ...
        parameters = ((parameters - self.mean) / self.std).astype(self.real_dtype)
